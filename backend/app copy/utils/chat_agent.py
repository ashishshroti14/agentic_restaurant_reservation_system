...
# Intent patterns
FIND_RESTAURANT_PATTERN = re.compile(r'(find|search|looking for|suggest|recommend).*restaurant', re.IGNORECASE)
CHECK_AVAILABILITY_PATTERN = re.compile(r'(check|available|availability|open|book)', re.IGNORECASE)
MAKE_RESERVATION_PATTERN = re.compile(r'(make|book|reserve|reservation)', re.IGNORECASE)
RETRIEVE_RESERVATION_PATTERN = re.compile(r'(find|get|show|view|my|list|retrieve)(\s+my)?(\s+reservations?)', re.IGNORECASE)
MODIFY_RESERVATION_PATTERN = re.compile(r'(change|update|modify|edit|reschedule)(\s+my)?(\s+reservations?)', re.IGNORECASE)
...
def detect_intent(message: str, session: Optional[ChatSession] = None) -> str:
    """Detect the user's intent from their message using LLM with full conversation context"""
    ...
    # Use LLM for intent classification with updated intent categories
    try:
        client = OpenAI(
        ...
        # Create a comprehensive prompt for intent classification with full conversation context
        intent_prompt = f"""
        ...
        Based on the full conversation context, classify the user's current intent into exactly one of these categories:
        ...
        Your response must be EXACTLY ONE of these categories: find_restaurant, check_availability, make_reservation, retrieve_reservation, modify_reservation, general_inquiry
        """
        
        # Log that we're making an intent detection LLM call
        logger.info(f"Making intent detection LLM call for message: {message}")
        ...
        completion = client.chat.completions.create(
            model=DEFAULT_MODEL,
            messages=[
                {"role": "system", "content": "You are an intent classification system for a restaurant reservation platform. Only respond with the exact intent category name."},
        ...
        log_llm_interaction(
            ...
            metadata={"operation": "intent_detection", "session_id": session.id if session else None}
        )
        
        return intent if intent in ["find_restaurant", "check_availability", "make_reservation", 
                                     "retrieve_reservation", "modify_reservation", 
                                    "general_inquiry"] else "general_inquiry"
            
    except Exception as e:
        # Log the error and fall back to regex matching
        logger.error(f"Error in LLM intent detection: {str(e)}", exc_info=True)
        
        # Fallback to regex-based detection
        if RETRIEVE_RESERVATION_PATTERN.search(message):
            return "retrieve_reservation"
        elif MODIFY_RESERVATION_PATTERN.search(message):
            return "modify_reservation"
        elif FIND_RESTAURANT_PATTERN.search(message):
            return "find_restaurant"
        elif CHECK_AVAILABILITY_PATTERN.search(message):
            return "check_availability"
        elif MAKE_RESERVATION_PATTERN.search(message):
            return "make_reservation"
        else:
            return "general_inquiry"

def extract_parameters(message: str, intent: str, session: Optional[ChatSession] = None) -> Dict[str, Any]:
    """Extract parameters from the user message based on intent using LLM with full conversation context"""
    ...
    try:
        ...
        if intent == "find_restaurant":
            ...
            """
        elif intent in ["check_availability", "make_reservation"]:
            extraction_prompt = f"""
            {system_context if system_context else ""}
            ...
            # For retrieve_reservation, always try to use the phone number from context
            extraction_prompt = f"""
            {system_context if system_context else ""}
        ...
        logger.info(f"Making parameter extraction LLM call for intent: {intent} with full context")
        ...
        log_llm_interaction(
            ...
            execution_time=execution_time,
            metadata={"operation": "parameter_extraction", "intent": intent}
        )
        ...
        # IMPORTANT: Ensure phone number is always added for reservation-related intents
        # This is critical for ensuring the phone number is used in tool calls
        if session and session.phone_number and intent in ["make_reservation", "retrieve_reservation", "modify_reservation"]:
            if "phone" not in cleaned_params or not cleaned_params.get("phone"):
                cleaned_params["phone"] = session.phone_number
                logger.info(f"Added session phone {session.phone_number} to parameters for {intent}")
            
            # Also ensure it's in the correct format for get_customer_reservations
            if intent in ["retrieve_reservation"]:
                cleaned_params["identifier_type"] = "phone"
                cleaned_params["identifier_value"] = session.phone_number
                logger.info(f"Set identifier to phone {session.phone_number} for {intent}")
...
def generate_response(session: ChatSession, user_message: str, restaurant_id: Optional[str] = None) -> Tuple[str, Optional[List[str]], str]:
    """Generate a response using the model from OpenRouter and return detected intent"""
    if not OPENROUTER_API_KEY:
        # If no API key, use rule-based responses
        intent = detect_intent(user_message)
        params = extract_parameters(user_message, intent)
        response_text = "I'm sorry, but I don't have access to the restaurant database right now. Please try again later."
        suggested_actions = None
        return response_text, suggested_actions, intent

    # Get intent with full conversation context
    intent = detect_intent(user_message, session)
    
    # Extract parameters with full conversation context
    params = extract_parameters(user_message, intent, session)
...
"""
    ...
    [TOOL:modify_reservation(reservation_id="12345", phone_number="1234567890", date_utc="2023-06-01T19:00:00")]

    - For retrieve_reservation:
    [TOOL:retrieve_reservation(identifier_type="phone", identifier_value="1234567890", date="2023-06-01")]
    ...
    try:
        ...
        while tool_call_count < max_tool_calls:
            ...
            log_llm_interaction(
                ...
                response=response_text,
                execution_time=execution_time,
                metadata={"intent": intent, "session_id": session.id, "tool_call_count": tool_call_count}
            )
        ...
        return final_response, suggested_actions, intent
        
    except Exception as e:
        logger.error(f"Error during response generation: {str(e)}", exc_info=True)
        return f"Sorry, I encountered an error: {str(e)}", None, intent

def perform_action(intent: str, params: Dict[str, Any], session_id: str) -> str:
    """Perform an action based on the detected intent and parameters"""
    # Validate inputs before processing
    if not isinstance(intent, str) or not isinstance(params, dict) or not isinstance(session_id, str):
...
def process_user_message(
    ...
    """Process a user message and generate a response, also returning detected intent"""
    ...
    # Generate response
    response_text, suggested_actions, intent = generate_response(session, user_message, restaurant_id)
    ...
    return session, assistant_msg, suggested_actions, intent
