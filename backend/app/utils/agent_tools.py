...
class AgentTools:
    ...
    def modify_reservation(
        ...
        Args:
            ...
            party_size: Optional new party size (integer)
            reservation_time: Optional new reservation time in ISO format (YYYY-MM-DDTHH:MM:SS)
        ...
        try:
            ...
            if party_size is not None:
                logger.info(f"MODIFY: Will update party_size to {party_size}")
            if reservation_time is not None:
                logger.info(f"MODIFY: Will update reservation_time to {reservation_time}")
            
            # First check if the reservation exists
            existing_reservation = get_reservation(reservation_id)
            if not existing_reservation:
                logger.error(f"MODIFY: Reservation with ID {reservation_id} not found")
                return json.dumps({
                    "status": "not_found",
            ...
            logger.info(f"MODIFY: Original reservation data: {updated_data}")

            # Update party size if provided
            if party_size is not None:
                updated_data['party_size'] = party_size
            
            # Update reservation time if provided
            if reservation_time is not None:
                try:
                    # Parse the reservation_time string into a datetime object
                    if isinstance(reservation_time, str):
                        # Clean any timezone indicators
                        if reservation_time.endswith('Z'):
                            reservation_time = reservation_time[:-1]
                        if '+' in reservation_time:
                            reservation_time = reservation_time.split('+')[0]
                        
                        # Try to parse the datetime
                        dt_obj = datetime.fromisoformat(reservation_time)
                        updated_data['reservation_time'] = dt_obj
                        logger.info(f"MODIFY: Parsed reservation_time to {dt_obj}")
                    else:
                        updated_data['reservation_time'] = reservation_time
                except Exception as dt_error:
                    logger.error(f"MODIFY: Failed to parse reservation_time: {str(dt_error)}")
                    return json.dumps({
                        "status": "error",
                        "message": f"Invalid reservation time format: {str(dt_error)}"
                    })
            
            # Log what we're about to send to the database
            logger.info(f"MODIFY: Updating reservation with new data: {updated_data}")

            # Update the reservation
            try:
                result = update_reservation(updated_data)
                logger.info(f"MODIFY: Update result: {result}")
                ...
                if result:
                    ...
                    # Try to fetch the reservation again to see if it was updated
                    logger.warning(f"MODIFY: update_reservation returned None, checking if update succeeded anyway")
                    fetched_reservation = get_reservation(reservation_id)
                    
                    # Check if the update was actually applied
                    if fetched_reservation:
                        # Check if party_size was updated
                        if party_size is not None and fetched_reservation.get('party_size') == party_size:
                            logger.info(f"MODIFY: Update succeeded despite None return - party_size matches")
                            return json.dumps({
                                "status": "success",
                                "message": "Reservation updated successfully (verified after update)",
                                "reservation": fetched_reservation
                            })
                        
                        # Check if reservation_time was updated (approximately)
                        if reservation_time is not None:
                            logger.info(f"MODIFY: Checking if reservation_time was updated")
                            logger.info(f"MODIFY: Fetched time: {fetched_reservation.get('reservation_time')}")
                            # This is a simplified check and might need refinement
                            if 'reservation_time' in fetched_reservation:
                                return json.dumps({
                                    "status": "success",
                                    "message": "Reservation updated successfully (verified after update)",
                                    "reservation": fetched_reservation
                    ...
                    # If we can't verify the update succeeded, return an error
                    logger.error(f"MODIFY: update_reservation returned None and update could not be verified")
                    return json.dumps({
                        "status": "error",
                        "message": "Failed to update reservation - database returned no result. The reservation may exist but the update failed."
                    })
            except Exception as db_error:
                logger.error(f"MODIFY: Database error during update: {str(db_error)}", exc_info=True)
                return json.dumps({
                    "status": "error",
                    "message": f"Database error during update: {str(db_error)}"
                })
                
        except Exception as e:
            logger.error(f"MODIFY: Error modifying reservation: {str(e)}", exc_info=True)
            return json.dumps({
                "status": "error",
...
# # Example usage of the tools - corrected version
# if __name__ == "__main__":
#     # Access the tool through the AVAILABLE_TOOLS dictionary
#     modify_tool = AVAILABLE_TOOLS["modify_reservation"]
    
#     # Call the tool as a function with the appropriate parameters
#     result = modify_tool(
#         reservation_id="res_8385a840", 
#         update_fields={"party_size": 8, "reservation_time": "2023-06-07T20:00:00"}
#     )
#     print(result)  # Print the JSON response
    
#     # Example of accessing another tool
#     get_details_tool = AVAILABLE_TOOLS["get_restaurant_details"]
#     details_result = get_details_tool(restaurant_id="sample_restaurant_id")
